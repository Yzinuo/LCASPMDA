...
def auc(outputs, labels,num):
    assert labels.dim() == 1 and outputs.dim() == 1
    labels = labels.detach().cpu().numpy()
    outputs = outputs.detach().cpu().numpy()
    fpr, tpr, _ = roc_curve(labels, outputs)
    num = str(num)
    np.savetxt("./data/MDAD/fpr_tpr/fpr" + num + ".txt", fpr)
    np.savetxt("./data/MDAD/fpr_tpr/tpr" + num + ".txt", tpr)

    return roc_auc_score(labels, outputs)

def aupr(outputs, labels,num):
    assert labels.dim() == 1 and outputs.dim() == 1
    labels = labels.detach().cpu().numpy()
    outputs = outputs.detach().cpu().numpy()
    precision, recall, _ = precision_recall_curve(labels, outputs)
    np.savetxt("./data/MDAD/PR/precision" + str(num) + ".txt", precision)
    np.savetxt("./data/MDAD/PR/recall" +str(num) + ".txt", recall)

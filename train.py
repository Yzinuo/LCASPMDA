...
###############################################################
# data
parser.add_argument('--dataset', type=str, default='MDAD',  # drugvirus  MDAD aBiofilm
                    help='dataset name')
parser.add_argument('--drug_num', type=int, default=1373,  # 175 1373 1720
                    help='drug node num')
parser.add_argument('--microbe_num', type=int, default=173,  # 95 173 140
...
def train(epoch, link_dti_id_train, train_dti_inter_mat, feats, sn_edge):
    ...
    output, encoder_embeds = model(protein_index, drug_index, feats,  sn_edge)
    preds = output
    Loss = nn.BCELoss()
...
def test(link_dti_id_test, test_dti_inter_mat, feats, pos, sn_edge):
    ...
    with torch.no_grad():
        ...
        drug_index_all = drug_index_all.repeat(1, args.microbe_num)
        # drug_index_all = drug_index_all.repeat(1,args.microbe_num).cuda()
        protein_index_all = torch.arange(len(feats) - len(pos[0])).repeat(args.drug_num, 1)
        ...
        output, encoder_embeds = model(protein_index_all, drug_index_all, feats, sn_edge)


        preds = output.reshape(args.drug_num, args.microbe_num)
        output1 = output.reshape(args.drug_num, args.microbe_num)
    ...
    return acc_dti_test, loss_test, preds, targets, output,output1, encoder_embeds
...
for train_times in range(fold_num):
    ...
    model = MDA_Graph(nn_nlayers = args.nn_nlayers, hidden_dim = args.hidden_dim,DTI_nn_nlayers = args.DTI_nn_nlayers,out_channels = 256,
                             add_self_loops = args.add,
                             heads = args.head,
                             heads_aggr = 'mean',
                             mode = args.mode,
                             att_version = args.version,
    ...
    for epoch in range(args.epochs):
        ...
        # Test
        # dti_inter_mat = ori_dti_inter_mat
        # train_interact_pos = ori_train_interact_pos
        # val_interact_pos  = ori_val_interact_pos
        ...
        train(epoch, train_interact_pos, dti_inter_mat, adj_similiarty,sn_edge )
        test_score, test_loss, predicts, targets, output,output1, encoder_embeds = test(val_interact_pos, dti_inter_mat, adj_similiarty, poss,
                                                                sn_edge)
    ...
    test_score, test_loss, predicts, targets, output2,output1, encoder_embeds = test(ori_val_interact_pos, ori_dti_inter_mat,
                                                    adj_similiarty, poss,sn_edge)
    fold_test = test_score
    fold_acc_score[train_times] = round(accuracy(predicts,targets), 4)
    fold_auc_score[train_times] = round(auc(predicts, targets,train_times), 4)
    fold_aupr_score[train_times] = round(aupr(predicts, targets,train_times), 4)

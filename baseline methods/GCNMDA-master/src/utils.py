...
def load_data(train_arr, test_arr):
    ...
    DS=np.loadtxt("E:\\实验室\\GCNMDA-master\\data\\MDAD\\drugsimilarity.txt")
    MS=np.loadtxt("E:\\实验室\\GCNMDA-master\\data\\MDAD\microbesimilarity.txt")
    adj = np.vstack((np.hstack((DS,M)),np.hstack((M.transpose(),MS))))   
                
    F1 = np.loadtxt("E:\\实验室\\GCNMDA-master\\data\\MDAD\\drugfeatures.txt")
    F2 = np.loadtxt("E:\\实验室\\GCNMDA-master\\data\\MDAD\\microbefeatures.txt")

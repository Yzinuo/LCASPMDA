...
def load_data(train_arr, test_arr):
    ...
    logits_test = sp.csr_matrix((labels[test_arr,2],(labels[test_arr,0]-1, labels[test_arr,1]-1)),shape=(1720,140)).toarray()
    logits_test = logits_test.reshape([-1,1])  

    logits_train = sp.csr_matrix((labels[train_arr,2],(labels[train_arr,0]-1, labels[train_arr,1]-1)),shape=(1720,140)).toarray()
    ...
    M = sp.csr_matrix((labels[train_arr,2],(labels[train_arr,0]-1, labels[train_arr,1]-1)),shape=(1720,140)).toarray()
    
    DS=np.loadtxt("E:\\实验室\\GCNMDA-master\\data\\aBiofilm\\drugsimilarity.txt")
    MS=np.loadtxt("E:\\实验室\\GCNMDA-master\\data\\aBiofilm\\microbesimilarity.txt")
    adj = np.vstack((np.hstack((DS,M)),np.hstack((M.transpose(),MS))))   
                
    F1 = np.loadtxt("E:\\实验室\\GCNMDA-master\\data\\aBiofilm\\drugfeatures.txt")
    F2 = np.loadtxt("E:\\实验室\\GCNMDA-master\\data\\aBiofilm\\microbefeatures.txt")
    features = np.vstack((np.hstack((F1,np.zeros(shape=(1720,140),dtype=int))), np.hstack((np.zeros(shape=(140,1720),dtype=int), F2))))
...
def generate_mask(labels,N):   
    num = 0
    A = sp.csr_matrix((labels[:,2],(labels[:,0]-1, labels[:,1]-1)),shape=(1720,140)).toarray()
    mask = np.zeros(A.shape)
    label_neg=np.zeros((5*N,2))  
    while(num<5*N):
        a = random.randint(0,1719)
        b = random.randint(0,139)
...
def adj_to_bias(sizes=[1860], nhood=1):
      
    labels = np.loadtxt("E:\\实验室\\GCNMDA-master\\data\\aBiofilm\\adj.txt")
    reorder = np.arange(labels.shape[0])
    train_arr=reorder.tolist()
    M = sp.csr_matrix((labels[train_arr,2],(labels[train_arr,0]-1, labels[train_arr,1]-1)),shape=(1720,140)).toarray()
    adj = np.vstack((np.hstack((np.zeros(shape=(1720,1720),dtype=int),M)),np.hstack((M.transpose(),np.zeros(shape=(140,140),dtype=int)))))

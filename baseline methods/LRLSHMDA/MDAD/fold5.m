...
interaction1=A;
    if cv<5
        B1=known(x1((cv-1)*floor(pp1/5)+1:floor(pp1/5)*cv),:);
        B2=unknown(x2((cv-1)*floor(pp2/5)+1:floor(pp2/5)*cv),:);
        for i=1:floor(pp1/5)
            interaction1(B1(i,1),B1(i,2))=0;
        end
    else
        B1=known(x1((cv-1)*floor(pp1/5)+1:pp1),:);
        B2=unknown(x2((cv-1)*floor(pp2/5)+1:pp2),:);
        for i=1:pp1-floor(pp1/5)*4
            interaction1(B1(i,1),B1(i,2))=0;
        end
    end
    for i=1:nr
